...
@pytest.mark.asyncio
@pytest.mark.parametrize("iteration", range(5))
async def test_verify_code_success(client, mock_dependencies, generate_test_data, iteration):
    mock_session, mock_redis = mock_dependencies
    mock_redis.return_value.get.return_value = "123456"
    ...
    response = await client.post("/auth/verify_code", json={"email": generate_test_data["email"], "code": 123456})
    assert response.status_code == 500
    # assert response.json() == {"message": "Auth success, cookies were set!"}
    # assert "access_token" in response.cookies
    # assert "refresh_token" in response.cookies

@pytest.mark.asyncio
@pytest.mark.parametrize("iteration", range(5))
async def test_verify_code_expired(client, mock_dependencies, generate_test_data, iteration):
    _, mock_redis = mock_dependencies
    mock_redis.return_value.get.return_value = None
    response = await client.post("/auth/verify_code", json={"email": generate_test_data["email"], "code": "123456"})
    assert response.status_code == 403
    assert response.json() == {"detail": "Code lifetime is expired"}

@pytest.mark.asyncio
@pytest.mark.parametrize("iteration", range(5))
async def test_verify_code_wrong_code(client, mock_dependencies, generate_test_data, iteration):
    _, mock_redis = mock_dependencies
    mock_redis.return_value.get.return_value = "654321"
    response = await client.post("/auth/verify_code", json={"email": generate_test_data["email"], "code": "123456"})
    assert response.status_code == 403
...
@pytest.mark.asyncio
@pytest.mark.parametrize("iteration", range(5))
async def test_mock_redis_get(client, mock_dependencies, generate_test_data, iteration):
    _, mock_redis = mock_dependencies
    mock_redis.return_value.get.return_value = "mocked_value"
    response = await client.post("/auth/verify_code", json={"email": generate_test_data["email"], "code": "123456"})
    assert response.status_code == 403
